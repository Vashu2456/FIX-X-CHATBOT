...
from VASHU X CHATBOT.modules.helpers import CHATBOT_ON, is_admins
...
async def chatbot_text(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        VASHU X CHATBOTdb = MongoClient(MONGO_URL)
        VASHU X CHATBOT  = VASHU X CHATBOTdb["VASHU X CHATBOTDb"]["VASHU X CHATBOT"]
        is_VASHU X CHATBOT  = VASHU X CHATBOT.find_one({"chat_id": message.chat.id})
        if not is_annie:
...
async def chatbot_sticker(client: Client, message: Message):
    ...
    if message.reply_to_message:
        VASHU X CHATBOTdb = MongoClient(MONGO_URL)
        VASHU X CHATBOT  = VASHU X CHATBOTdb["AnnieDb"]["Annie"]
        is_VASHU X CHATBOT  = VASHU X CHATBOT.find_one({"chat_id": message.chat.id})
        if message.reply_to_message.from_user.id == Client.id:
            if not is_annie:
                await client.send_chat_action(message.chat.id, ChatAction.TYPING)
