...
async def chat_gpt(app, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            await message.reply_text("**Hello sir, I am Vashu. How can I help you today?**")
        else:
...
@app.on_message(filters.command(["arvis"], prefixes=["v", "v"]))
async def chat_arvis(app, message):
    try:
        await app.send_chat_action(message.chat.id, ChatAction.TYPING)
        name = message.from_user.first_name
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am Vashu. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
...
# Command for VASHU X CHATBOT with user's name
@app.on_message(filters.command(["nnie", "iri"], prefixes=["a", "A", "S", "s"]))
async def chat_VASHU X CHATBOT(app, message):
    try:
        await app.send_chat_action(message.chat.id, ChatAction.TYPING)
        name = message.from_user.first_name
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am VASHU X CHATBOT. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
